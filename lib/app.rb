...
def send_twitt(twitt,handles)
    client = login
      handles.sample(5).each do |items| 
        client.update(twitt + " " + items)
      end
end

def like_bonjour(hashtag)
    client = login
    client.search(hashtag).take(25).each do |tweet|
    client.fav(tweet)
    end
end


def follow_user_tweet_hashtag(hashtag,nb_of_users)
client = login

client.search(hashtag).take(nb_of_users).each do |tweet|
...
def live_tweet(hashtag)
  client = login_streaming_api
  client2 = login
